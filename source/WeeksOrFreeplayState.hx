...
class WeeksOrFreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-125).loadGraphic(Paths.loadImage('menuBG_StoryOrFreeplay'));
		...
		magenta = new FlxSprite(-80).loadGraphic(Paths.loadImage('menuBG_StoryOrFreeplay'));
		magenta.scrollFactor.x = 0.10;
		...
		for (i in 0...optionShit.length)
		{
			...
			if (firstStart)
				FlxTween.tween(menuItem, {x: 100 + (i * 800), y: 600 - (i * 550)}, 1 + (i * 0.25), {
			...
			{
				menuItem.x = 100 + (i * 800);
				menuItem.y = 600 - (i * 550);
			}
		}
